...
export class AdminService {
  ...
  showRooms(hotelId:number): Observable<any> {
    return this.http.get('http://localhost:8888/rooms-admin/hotelName/'+hotelId);
  }

  addHotel(hotel: Hotel): Observable<any> {
    return this.http.post('http://localhost:8888/hotel/add' ,hotel);
  }

  updateHotel(hotel: Hotel): Observable<any> {
    return this.http.put('http://localhost:8888/hotel/update' ,hotel);
  }

  addRoom(room: Room): Observable<any> {
    return this.http.post('http://localhost:8888/room/add' ,room);
  }

  deleteHotel(hotelId: number): Observable <any> {
    console.log("inside delete Hotel of admin service "+hotelId)
    return this.http.delete('http://localhost:8888/hotel/delete/'+hotelId);
  }

  deleteRoom(roomId: number): Observable <any> {
    console.log("inside delete Room of admin service "+roomId)
    return this.http.delete('http://localhost:8888/room/delete/'+roomId);
  }
  ...
  getAllBookings(): Observable<any> {
    return this.http.get('http://localhost:8888/bookings');
  }

  getBookingsByHotel(hotelId:number): Observable<any> {
    return this.http.get('http://localhost:8888/bookings/by/hotel/'+hotelId);
  }
  ...
  getAllUsers(): Observable<any> {
    return this.http.get('http://localhost:8888/users/loginDetails');
  }
  ...
  getBookingsByDate(startDate:string,endDate:string): Observable<any> {
    return this.http.get('http://localhost:8888/bookings/date/'+startDate+'/to/'+endDate);
  }
}
